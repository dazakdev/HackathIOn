        ...
        return (
          <div key={idx} className="flex flex-col items-center gap-3">
            ...
            <div className="size-20 rounded-2xl bg-muted border border-border flex items-center justify-center">
              <div className="size-12 rounded-full border-2 border-border bg-card" />
            </div>
    ...
    <div className="space-y-3">
      {rest.length > 0 ? (
        rest.map((entry, idx) => (
          <div
            key={entry.id}
            className="flex items-center justify-between rounded-full bg-muted/60 px-4 py-2 text-sm"
          >
            <span className="text-xs text-muted-foreground">{idx + 4}. {entry.full_name ?? entry.email.split("@")[0]}</span>
            <span className="rounded-full bg-card px-3 py-1 text-[11px] font-semibold">
      ...
      <div className="flex items-center justify-between rounded-full bg-muted/60 px-4 py-2 text-sm">
        <span className="text-xs text-muted-foreground">233123. Your Name</span>
        <span className="rounded-full bg-card px-3 py-1 text-[11px] font-semibold">120 XP</span>
      </div>
    </div>
  )
  ...
  return (
    <div className="mx-auto max-w-[980px] space-y-10 pb-10">
      {isLoading ? (
        ...
        <>
          <section className="rounded-3xl border border-border bg-card shadow-[0_16px_40px_rgba(15,12,24,0.08)] p-8">
