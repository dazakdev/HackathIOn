  ...
  return (
    <div className="mx-auto max-w-[1180px] space-y-10 pb-12">
      <div className="flex items-center justify-end gap-3">
        <div className="rounded-full bg-card px-4 py-2 text-sm font-semibold shadow-sm border border-border">
          {(user?.total_points ?? 12450).toLocaleString("pl-PL")} XP
        </div>
        <button className="size-9 rounded-full border border-border bg-card shadow-sm flex items-center justify-center">
      ...
      <section className="rounded-3xl border border-border bg-card shadow-[0_18px_40px_rgba(15,12,24,0.08)] relative overflow-hidden">
        <div className="absolute right-0 top-1/2 -translate-y-1/2 size-72 rounded-full border border-border/60" />
        <div className="relative p-10 space-y-6 max-w-3xl">
          ...
          <Link to="/games/new">
            <Button className="rounded-xl bg-primary px-6 py-5 text-sm font-semibold shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {active.slice(0, 2).map((g) => (
              <div key={g.id} className="rounded-2xl border border-border bg-card shadow-[0_12px_30px_rgba(15,12,24,0.06)] p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="size-9 rounded-full bg-muted flex items-center justify-center">
                    <BookOpen className="h-4 w-4" />
                  </div>
                  <span className="text-[11px] uppercase tracking-widest bg-muted px-3 py-1 rounded-full text-muted-foreground">
      ...
      <section className="space-y-4">
        ...
        <div className="rounded-2xl border border-border bg-card shadow-[0_12px_30px_rgba(15,12,24,0.06)] p-6 max-w-md">
